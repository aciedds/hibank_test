import 'package:hibank_test/data/source/auth_remote_source.dart';
import 'package:hibank_test/domain/entity/auth/auth_data.dart';
import 'package:hibank_test/domain/mapper/auth_mapper.dart';
...
class AuthRepositoryImpl implements AuthRepository {
  ...
  @override
  Future<DataState<AuthData>> signInWithGoogle() async {
    final result = await _remote.signInWithGoogle();
    return result.when(
      success: (data) =>
          DataState.success(data: _mapper.mapModelToEntity(data)),
      error: (message, data, exception, stackTrace, statusCode) {
        return DataState.error(message: message);
      },
    );
